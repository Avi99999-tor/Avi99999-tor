...
def clean_and_validate_history(raw_text): entries = raw_text.strip().split() cleaned = [] errors = [] for val in entries: val = val.upper().replace("O", "0").replace(",", ".") if not val.endswith("X"): val += "X" try: float(val.replace("X", "")) cleaned.append(val) except: errors.append(val) return cleaned, errors

--- Prédiction ---

def calculate_predictions(cleaned_history, last_tour_number): multipliers = [float(x.replace("X", "")) for x in cleaned_history] predictions = [] for i in range(1, 21): recent = multipliers[-5:] avg = np.mean(recent) std = np.std(recent) mod_vals = [int(str(x).split(".")[-1]) % 10 for x in recent if '.' in str(x)] mod_score = sum([1 for m in mod_vals if m in [2, 4, 7]])
...
st.set_page_config(page_title="Prediction Expert by Mickael", layout="centered") st.markdown("### Prediction Expert by Mickael") st.markdown("Admin: Mickael  |  Contact: 033 31 744 68") st.markdown("---")
...
else: st.success("Connecté en tant que Topexacte") st.markdown("### Entrer l'historique des multiplicateurs") st.markdown("Format: 1.22x 3.45x 1.00x ...")
