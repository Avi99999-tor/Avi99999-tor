...
def mod10_strategy(multipliers):
    results = []
    for i, m in enumerate(multipliers):
        val = convert_to_int(m)
        mod_val = val % 10
...
# Streamlit UI
st.set_page_config(page_title="Aviator Mod10 Prediction", page_icon="✈️", layout="centered")
st.title("✈️ Aviator Prediction - Mod10 Strategy")
...
if input_values:
    try:
        raw_list = [float(x.strip()) for x in input_values.split(',')]
        results = mod10_strategy(raw_list)
